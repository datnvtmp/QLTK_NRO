...
public class GameScr : mScreen, IChatable
{
    ...
    public static void loadCamera(bool fullmScreen, int cx, int cy)
    {
        ...
        {
            GameCanvas.panel.isViewChatServer = false;
        }
        else
        {
            GameCanvas.panel.isViewChatServer = false;
    ...
    private void paintInfoBar(mGraphics g)
    {
        ...
        else
        {
            ...
            else if (Char.myCharz().mobFocus != null)
            {
                ...
                {
                    mFont.tahoma_7b_green2.drawString(g, Char.myCharz().mobFocus.getTemplate().name 
                        +" [" + Char.myCharz().mobFocus.mobId + "]", imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
                }
