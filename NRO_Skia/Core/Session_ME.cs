	...
	public void doConnect(string host, int port)
	{
        ...
        if (!string.IsNullOrEmpty(rawProxy) && rawProxy != "None")
        {
            ...
            if (parts.Length >= 4)
            {
                ...
                sc.Connect(proxyIp, proxyPort);
                var stream = sc.GetStream();

                // HTTP CONNECT tunnel — giống Java cũ
                string auth = Convert.ToBase64String(Encoding.ASCII.GetBytes($"{proxyUser}:{proxyPass}"));
                ...
                {
                    GameClient.Send(MsgType.DATA_INGAME, "Proxy lỗi - dừng kết nối");
                    throw new Exception($"Proxy lỗi: {response.Trim()}");
                }
            }
        }
        else
