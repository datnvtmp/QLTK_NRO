...
public class Panel : IActionListener, IChatable
{
    ...
    // Note: this field is not readonly, but it is never assigned to outside of a constructor, so effectively readonly.
    public static bool isGivingItemToPet = false;
    ...
    private void setTabInventory(bool resetSelect)
    {
        ...
        int total = Char.myCharz().arrItemBody.Length
                  + Char.myCharz().arrItemBag.Length;
        currentListLength = total;
        ITEM_HEIGHT = 24;
    ...
    private void paintSkill(mGraphics g)
    {
        ...
        for (int i = 0; i < num + 6; i++)
        {
            ...
            if (skill != null)
            {
                mFont.tahoma_7b_blue.drawString(g, "[" + skillTemplate.id + "] " + skillTemplate.name, num2 + 5, num3 + 3, 0);
    ...
    {
        Item[] body = Char.myCharz().arrItemBody;
        Item[] bag = Char.myCharz().arrItemBag;
        int total = body.Length + bag.Length;
        ...
        {
            bool isBody = i < body.Length;
            Item item = isBody ? body[i] : bag[i - body.Length];

            int ix = xScroll + 36;
            int iy = yScroll + i * ITEM_HEIGHT;
            int iw = wScroll - 36;
            int ih = ITEM_HEIGHT - 1;
            int icx = xScroll;
            int icy = iy;
            int icw = 34;
            int ich = ITEM_HEIGHT - 1;

            // Cắt bỏ những ô ngoài màn hình
            if (iy - cmy > yScroll + hScroll || iy - cmy < yScroll - ITEM_HEIGHT)
                continue;

            // Màu nền text area: body sáng hơn bag
            g.setColor(i == selected ? 16383818 : (isBody ? 15196114 : 15723751));
            g.fillRect(ix, iy, iw, ih);

            // Màu ô icon mặc định
            g.setColor(i == selected ? 9541120 : (isBody ? 9993045 : 11837316));

            // Override màu ô icon theo cấp độ upgrade
            if (item != null && item.itemOption != null)
            {
                for (int j = 0; j < item.itemOption.Length; j++)
                {
                    if (item.itemOption[j].optionTemplate.id == 72 && item.itemOption[j].param > 0)
                    {
                        sbyte cl = GetColor_Item_Upgrade(item.itemOption[j].param);
                        int bg = GetColor_ItemBg(cl);
                        if (bg != -1) g.setColor(bg);
                    }
                }
            }
            g.fillRect(icx, icy, icw, ich);

            // Highlight khi đang chọn để combine
            if (item != null && item.isSelect && GameCanvas.panel.type == 12)
            {
                g.setColor(i != selected ? 6047789 : 7040779);
                g.fillRect(icx, icy, icw, ich);
            ...
            // --- Tên item ---
            string upgradeText = string.Empty;
            ...
            if (item.itemOption != null)
            {
                ...
                {
                    if (item.itemOption[j].optionTemplate.id == 72)
                        upgradeText = " [+" + item.itemOption[j].param + "]";

                    if (item.itemOption[j].optionTemplate.id == 41)
                    {
                        if (item.itemOption[j].param == 1) nameFont = GetFont(0);
                        else if (item.itemOption[j].param == 2) nameFont = GetFont(2);
                        else if (item.itemOption[j].param == 3) nameFont = GetFont(8);
                        else if (item.itemOption[j].param == 4) nameFont = GetFont(7);
                    }
                }
            }
            ...
            if (item.itemOption != null)
            {
                ...
                if (item.itemOption.Length > 0
                    && item.itemOption[0] != null
                    && item.itemOption[0].optionTemplate.id != 102
                    && item.itemOption[0].optionTemplate.id != 107)
                {
                    statsText += item.itemOption[0].getOptionString();
                }

                mFont statsFont = mFont.tahoma_7_blue;
                if (item.compare < 0 && item.template.type != 5)
                    statsFont = mFont.tahoma_7_red;

                if (item.itemOption.Length > 1)
                {
                    // Chỉ lấy 1 option phụ (giống code gốc)
                    for (int j = 1; j < 2; j++)
                    {
                        if (item.itemOption[j] != null
                            && item.itemOption[j].optionTemplate.id != 102
                            && item.itemOption[j].optionTemplate.id != 107)
                        {
                            statsText += "," + item.itemOption[j].getOptionString();
                        }
                    }
                }
                statsFont.drawString(g, statsText, ix + 5, iy + 11, mFont.LEFT);
            }

            // --- Icon ---
            SmallImage.drawSmallImage(g, item.template.iconID,
                icx + icw / 2, icy + ich / 2, 0, 3);

            // --- Effect ô icon (upgrade glow, slot...) ---
            if (item.itemOption != null)
            {
                for (int j = 0; j < item.itemOption.Length; j++)
                    paintOptItem(g, item.itemOption[j].optionTemplate.id,
                        item.itemOption[j].param, icx, icy, icw, ich);

                for (int j = 0; j < item.itemOption.Length; j++)
                    paintOptSlotItem(g, item.itemOption[j].optionTemplate.id,
                        item.itemOption[j].param, icx, icy, icw, ich);
            }
            ...
            if (item.quantity > 1)
                mFont.tahoma_7_yellow.drawString(g,
                    string.Empty + item.quantity,
                    icx + icw,
                    icy + ich - mFont.tahoma_7_yellow.getHeight(), 1);

            // --- Badge NEW ---
            if (item.newItem && GameCanvas.gameTick % 10 > 5)
                g.drawImage(imgNew, icx + icw / 2, iy + 19, 3);
        }
    ...
    private void paintBottomMoneyInfo(mGraphics g)
    {
        if (type != 13 || (currentTabIndex != 2 && !Equals(GameCanvas.panel2)))
        {
            ...
            mFont.tahoma_7_yellow.drawString(g, AutoSendInFo.FormatPower(Char.myCharz().xu) + string.Empty, X + 24, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
            mFont.tahoma_7_yellow.drawString(g,AutoSendInFo.FormatPower( Char.myCharz().luong) + string.Empty, X + 85, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
            g.drawImage(imgLuongKhoa, X + 130, H - 8, 3);
            mFont.tahoma_7_yellow.drawString(g,AutoSendInFo.FormatPower( Char.myCharz().luongKhoa) + string.Empty, X + 140, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
        }
    }
    ...
    private void doFireShop()
    {
        ...
        {
            // thành (giống doFireInventory)
            Item[] body = Char.myCharz().arrItemBody;
            Item[] bag = Char.myCharz().arrItemBag;
            bool isBody = selected < body.Length;
            currItem = isBody ? body[selected] : bag[selected - body.Length];
            if (currItem != null)
            {
    ...
    private void doFireInventory()
    {
        ...
        if (currItem == null) { cp = null; return; }
        ...
        Char.myCharz().setPartTemp(
            currItem.headTemp, currItem.bodyTemp,
            currItem.legTemp, currItem.bagTemp);
    ...
    private void doFireBox()
    {
        ...
        {
            // xóa cái if (selected == 0) setNewSelected đi
            // đổi trực tiếp
        ...
        if (currentTabIndex == 1 || Equals(GameCanvas.panel2))
        {
            if (currentTabIndex == 1 || Equals(GameCanvas.panel2))
            {
                ...
                Item[] bag = Char.myCharz().arrItemBag;
                bool isBody = selected < body.Length;
                Item item2 = isBody ? body[selected] : bag[selected - body.Length];

                if (item2 != null)
                {
                    if (isBody)
                    {
                        myVector.addElement(new Command(mResources.move_to_chest2, this, 1002, item2));
                    }
                    else
                    {
    ...
    public void perform(int idAction, object p)
    {
        ...
        if (idAction == 2000)
        {
            Item[] b = Char.myCharz().arrItemBody;
            sbyte bagIdx = (sbyte)(selected - b.Length);
            Service.gI().getItem(BAG_BODY, bagIdx);
        }

        if (idAction == 2001)
        {
            Item item9 = (Item)p;
            Item[] b2 = Char.myCharz().arrItemBody;
            bool isB = selected < b2.Length;
            sbyte idx = isB ? (sbyte)selected : (sbyte)(selected - b2.Length);
            Service.gI().useItem(0, isB ? (sbyte)0 : (sbyte)1, idx, -1);
            if (item9.template.id == 193 || item9.template.id == 194)
                GameCanvas.panel.hide();
        }
        // ── 2002: tháo trang bị xuống túi ──────────────────────────────
        if (idAction == 2002)
        {
            // selected chính là index trong arrItemBody
            Service.gI().getItem(BODY_BAG, (sbyte)selected);
        }
        if (idAction == 2003)
        {
            Item[] b3 = Char.myCharz().arrItemBody;
            bool isB = selected < b3.Length;
            sbyte idx = isB ? (sbyte)selected : (sbyte)(selected - b3.Length);
            Service.gI().useItem(1, isB ? (sbyte)0 : (sbyte)1, idx, -1);
        }
        if (idAction == 2004)
        {
            GameCanvas.endDlg();
        ...
        if (idAction == 2005)
        {
            Item[] b4 = Char.myCharz().arrItemBody;
            sbyte bagI = (sbyte)(selected - b4.Length);
            Panel.isGivingItemToPet = true; // thêm dòng này
            Service.gI().getItem(BAG_PET, bagI);
        }
        if (idAction == 2006)
        {
        ...
        {
            GameCanvas.endDlg();
            Item[] b5 = Char.myCharz().arrItemBody;
            bool isB = selected < b5.Length;
            sbyte idx = isB ? (sbyte)selected : (sbyte)(selected - b5.Length);
            Service.gI().saleItem(0, isB ? (sbyte)0 : (sbyte)1, idx);
        }
        if (idAction == 3003)
        {
            GameCanvas.endDlg();
